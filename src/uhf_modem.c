...
//     unsigned char overall_attempts;
//     unsigned char rd_attempts;
//     int rd_sz;
//     int retval;
//     uhf_frame_t frame[1];

//     for (overall_attempts = 0; (overall_attempts < uhf_max_retries) && (rd_sz < UHF_MAX_FRAME_SIZE); overall_attempts++){

//         rd_sz = 0;
//         retval = 0;
//         memset(frame, 0x0, UHF_MAX_FRAME_SIZE);

//         for (rd_attempts = 0; (rd_attempts < uhf_max_retries) && (rd_sz < UHF_MAX_FRAME_SIZE) rd_attempts++){
//             if ((rd_sz == 3) && (frame[0] == 'O') && (frame[1] == 'K') && (frame[2] == '+')){
//                 eprintf("PIPE command received, resetting to read data!");
//                 break;
//             }

//             retval = read(dev, frame + rd_sz, UHF_MAX_FRAME_SIZE - rd_sz);
//             if (retval > 0){
//                 rd_sz += retval;
//             } else {
//                 eprintf("Error reading data!");
...
//         if ((overall_attempts >= uhf_max_retries) && (rd_sz < UHF_MAX_FRAME_SIZE)){

//         } else if (rd_sz < UHF_MAX_FRAME_SIZE) {
//             // You shouldn't be here.

//         } else if (frame->guid != UHF_GUID) {

//         } else if (frame->termination != UHF_TERMINATION){

//         } else if (frame->crc != frame->crc1) {

//         } else if (frame->crc == 0x660a) {

//         }

//         memset(frame->crc, 0x0, 1);
//         memset(frame->crc, 0x0, 1);
